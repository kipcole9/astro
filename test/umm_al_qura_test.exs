defmodule Astro.UmmAlQuraTest do
  @moduledoc """
  ExUnit tests for `Astro.UmmAlQura.first_day_of_month/2`.
  ...
  test "first_day_of_month/2 achieves 100% accuracy across the full dataset" do
    ...
    correct =
      Enum.count(reference_data, fn %{hijri_year: year, hijri_month: month, gregorian: expected} ->
        match?({:ok, ^expected}, UmmAlQura.first_day_of_month(year, month))
      end)
  ...
  describe "regression: months that failed with approximate_29th (~25% error rate)" do
    test "all 12 months of 1430 AH" do
      for month <- 1..12 do
        {:ok, result} = UmmAlQura.first_day_of_month(1430, month)
    ...
    test "all 12 months of 1440 AH" do
      for month <- 1..12 do
        {:ok, result} = UmmAlQura.first_day_of_month(1440, month)
    ...
    test "all 12 months of 1445 AH" do
      for month <- 1..12 do
        {:ok, result} = UmmAlQura.first_day_of_month(1445, month)
  ...
  describe "structural invariants" do
    ...
    test "month 12 (Dhu al-Hijja) 1445 AH is correctly bounded" do
      {:ok, result} = UmmAlQura.first_day_of_month(1445, 12)
  ...
  describe "error cases" do
    test "returns error for a year beyond the dataset" do
      assert {:error, :date_not_in_official_table} = UmmAlQura.first_day_of_month(9999, 1)
    end

    test "returns error for month 0" do
      assert {:error, :date_not_in_official_table} = UmmAlQura.first_day_of_month(1446, 0)
    end

    test "returns error for month 13" do
      assert {:error, :date_not_in_official_table} = UmmAlQura.first_day_of_month(1446, 13)
    end
  end
